...
async function createProfileBucket() {
    try {
        ...
        await db.query(`
            CREATE POLICY "Public Document Access" ON storage.objects FOR SELECT USING (bucket_id = 'profile-photos');
        `).catch(err => console.log("Policy might already exist:", err.message));

        // Allow authenticated inserts
        await db.query(`
            CREATE POLICY "Auth Insert Photos" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'profile-photos');
        `).catch(err => console.log("Policy might already exist:", err.message));
